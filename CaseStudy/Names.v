...
Section Names.
  ...
  Global Instance BotValue_Container : Container BotValue.
    apply (ConstContainer unit BotValue
            ...
            (fun A x => Bot _)); destruct x; reflexivity.
  Defined.
  ...
  | P_Nil : P_Env P nil
  | P_Cons a (As : Env _) : P a -> P_Env P As -> P_Env P (cons a As).
  ...
  Lemma P2_Env_lookup A B (env : Env A) (env' : Env B) P :
    P2_Env P env env' ->
    forall n v,
      lookup env n = Some v -> exists v', lookup env' n = Some v' /\ P v v'.
  Proof.
  ...
  Lemma Sub_Environment_refl (env : Env _) : Sub_Environment env env.
  Proof.
  ...
  Definition eval_continuous_Exp_P (e : Exp) :=
    forall (m : nat),
      (forall (e0 : Exp) (gamma gamma' : Env Value) (n : nat),
  ...
  Lemma WF_Value_beval m n (e : Exp) gamma gamma' T :
      Sub_Environment gamma' gamma ->
      m <= n ->
