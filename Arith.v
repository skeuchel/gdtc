...
Section Arith.
  ...
  Definition AType_fmap (A B : Set) (f : A -> B) :
    AType A -> AType B := fun _ => TNat _.
  ...
  Definition ind_alg_Arith
    ...
    match e with
      ...
      | Add m n => exist (UP'_P P) _
                         (H0 (proj1_sig m) (proj1_sig n)
                             (proj2_sig m) (proj2_sig n))
    end.
  ...
  Definition VI_fmap (A B : Set) (f : A -> B) :
      ...
      | Add m n =>
        fun env =>
          ...
          match isVI (proj1_sig m'), isVI (proj1_sig n') with
            ...
            | _, _ =>
              if @isBot _ Fun_V Sub_BotValue_V (proj1_sig m')
              then @bot' _ Fun_V Sub_BotValue_V
              else if @isBot _ Fun_V Sub_BotValue_V (proj1_sig n')
                   then @bot' _ Fun_V Sub_BotValue_V
                   else stuck' 4
          end
    end.
